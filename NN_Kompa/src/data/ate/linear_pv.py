...
def generatate_linear_core(n_sample: int, rng, beta_0: float, beta_a: float, beta_w: float,
                                delta_0: float, delta: float, gamma_0: float, gamma: float, lam_z: float, lam_u: float, 
    ...
    Z = gamma_0 + gamma * U + e_z
    A = lam_z * Z + lam_u * U + e_a
    Y = cal_outcome(A, W, U, beta_0, beta_a, beta_w)
    return U, W, Z, A, Y, e_w, e_z, e_a


def generate_train_linear_pv(n_sample: int, beta_0: float, beta_a: float, beta_w: float,  
                                  delta_0: float,  delta: float, gamma_0: float, gamma: float, lam_z: float, lam_u: float, 
                                  U_noise: float = 1, A_noise: float = 1,
                                  Z_noise: float = 1, W_noise: float = 1,
                                  seed=42, **kwargs):
    ...
    U, W, Z, A, Y, e_w, e_z, e_a = generatate_linear_core(n_sample, rng, beta_0, beta_a, beta_w, delta_0, delta, 
                                gamma_0, gamma, lam_z, lam_u, U_noise, A_noise, Z_noise, W_noise)
...
def cal_outcome(A, W, U, beta_0, beta_a, beta_w):
    return beta_0 + beta_a * A + beta_w * W + U
...
def generate_test_linear_pv(a_start, a_end, beta_0, beta_a, beta_w, delta_0, delta, U_noise, W_noise, **kwargs):
...
def calc_haw(A, W, e_w, beta_0, beta_a, beta_w, delta_0, delta, W_noise):
    ...
    delta_star = 1/delta
    beta_0_bar = beta_0 - delta_0/delta
    beta_w_star = beta_w + delta_star

    haw = beta_0_bar + beta_a * A + beta_w_star*W
    ha = beta_0_bar  + beta_a * A + np.mean(beta_w_star*W)
    return haw, ha
...
def gen_haw(a, beta_0, beta_a, beta_w, delta_0, delta, U_noise, W_noise):
    ...
    haw, ha = calc_haw(a, W, e_w, beta_0, beta_a, beta_w, delta_0, delta, W_noise)
    
    return haw, ha

def ha_effect(a_start, a_end, beta_0, beta_a, beta_w, delta_0, delta, U_noise, W_noise):
    a_steps = np.linspace(a_start, a_end, 100)
    ha = np.array([gen_haw(a, beta_0, beta_a, beta_w, delta_0, delta, U_noise, W_noise)[1] for a in a_steps])
    return a_steps, ha
