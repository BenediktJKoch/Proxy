...
#                 print("batch_x", batch_x.shape)
        ...
        val_loss = self.causal_val_losses[-1]
...
#         print("model", model_inputs_test.shape)
...
class NMMR_Trainer_MastourieExperiment(object):
    def __init__(self, data_configs: Dict[str, Any], train_params: Dict[str, Any], random_seed: int,
                 dump_folder: Optional[Path] = None):
    ...
    def train(self, train_t: PVTrainDataSetTorch, val_t: PVTrainDataSetTorch, verbose: int = 0) -> MLP_for_NMMR:
        ...
        # inputs consist of (A, W) tuples
        model = MLP_for_NMMR(input_dim=3, train_params=self.train_params)
...
#                     print("batch_Z", train_t.treatment_proxy[indices])
    ...
    @staticmethod
    def predict(model, test_data_t: PVTestDataSetTorch, val_data_t: PVTrainDataSetTorch):
        # I think it should be shape [intervention_array_len, n_samples, 3]
        # NOT:
        # Create a 3-dim array with shape [intervention_array_len, n_samples, 2]
        # This will contain the test values for do(A) chosen by Xu et al. as well as {n_samples} random draws for W
...
#         print("A", test_data_t.treatment.shape)
#         print("temp1", temp1.shape)
#         print("W", val_data_t.outcome_proxy.shape)
#         print("int", intervention_array_len)
        
        
        # temp2 = val_data_t.outcome_proxy.expand(-1, intervention_array_len)
        temp2 = np.repeat(val_data_t.outcome_proxy[np.newaxis, :, :], intervention_array_len, axis=0)
#         print("temp1/2", temp1.shape, temp2.shape)
        
#         model_inputs_test = torch.stack((temp1, temp2), dim=-1)
        model_inputs_test = torch.cat((temp1, temp2), 2)
